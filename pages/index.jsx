import { useEffect, useRef, useState } from 'react';
import ProgressBar from '../components/ProgressBar';
import GoalWizard from '../components/GoalWizard';
import HistoryChart from '../components/HistoryChart';

function formatDateKey(d = new Date()) { return d.toISOString().slice(0,10); }

// Extract nutrients robustly from USDA details
function extractNutrients(details) {
  // Prefer labelNutrients if present (branded foods)
  const label = details.labelNutrients || {};
  const pick = (obj, k) => Number(obj?.[k]?.value ?? obj?.[k]) || 0;

  let calories = pick(label, 'calories');
  let protein = pick(label, 'protein');
  let carbs = pick(label, 'carbohydrates');
  let fat = pick(label, 'fat');
  let fiber = pick(label, 'fiber');
  let sodium = pick(label, 'sodium');

  // If label missing, use foodNutrients by nutrientNumber
  const fallback = {208:'calories',203:'protein',205:'carbs',204:'fat',291:'fiber',307:'sodium',301:'calcium',303:'iron',306:'potassium',328:'vitaminD'};
  (details.foodNutrients || []).forEach(n => {
    const key = fallback[n.nutrientNumber];
    const val = Number(n.value ?? n.amount);
    if (key && Number.isFinite(val)) {
      if (key === 'calories') calories = calories || val;
      if (key === 'protein')  protein = protein  || val;
      if (key === 'carbs')    carbs   = carbs    || val;
      if (key === 'fat')      fat     = fat      || val;
      if (key === 'fiber')    fiber   = fiber    || val;
      if (key === 'sodium')   sodium  = sodium   || val;
    }
  });

  return {
    calories: Number(calories) || 0,
    protein: Number(protein) || 0,
    carbs: Number(carbs) || 0,
    fat: Number(fat) || 0,
    fiber: Number(fiber) || 0,
    sodium: Number(sodium) || 0,
  };
}

// Parse voice transcript into items
function parseVoiceItems(transcript) {
  if (!transcript) return [];
  const parts = transcript.split(/,| and /i).map(s=>s.trim()).filter(Boolean);
  const wordNums = {one:1,two:2,three:3,four:4,five:5,six:6,seven:7,eight:8,nine:9,ten:10,half:0.5};
  return parts.map(phrase=>{
    const m = phrase.match(/^(\d+(?:\.\d+)?)\s*(g|gram|grams|kg|cup|cups|tbsp|tablespoon|tsp|teaspoon|slice|slices)?\s*(.*)$/i);
    let qty=1, unit='serving', name=phrase;
    if (m) { qty = parseFloat(m[1]); unit = m[2] || 'serving'; name = m[3] || ''; }
    else {
      const wm = phrase.match(/^(one|two|three|four|five|six|seven|eight|nine|ten|half)\s+(.*)/i);
      if (wm) { qty = wordNums[wm[1].toLowerCase()] || 1; name = wm[2]; unit='serving'; }
      else { name = phrase; }
    }
    return { qty, unit, name: name.trim() };
  });
}

export default function Home() {
  const [query, setQuery] = useState('');
  const [searchResults, setSearchResults] = useState([]);
  const [qty, setQty] = useState(1);
  const [unit, setUnit] = useState('serving');
  const [log, setLog] = useState(()=>{ try { return JSON.parse(localStorage.getItem('qf_log')||'{}'); } catch { return {}; }});
  const [goals, setGoals] = useState(()=>{ try { return JSON.parse(localStorage.getItem('qf_goals')||'null'); } catch { return null; }});
  const todayKey = formatDateKey();
  const [listening, setListening] = useState(false);
  const recognitionRef = useRef(null);

  useEffect(()=>{ localStorage.setItem('qf_log', JSON.stringify(log)); }, [log]);
  useEffect(()=>{ localStorage.setItem('qf_goals', JSON.stringify(goals)); }, [goals]);

  // Search
  useEffect(()=>{
    const controller = new AbortController();
    if (!query) { setSearchResults([]); return; }
    const t = setTimeout(async ()=>{
      try {
        const res = await fetch(`/api/search?q=${encodeURIComponent(query)}`, { signal: controller.signal });
        const data = await res.json();
        setSearchResults(data.items || []);
      } catch(e){ console.error('search error', e); }
    }, 250);
    return ()=>{ clearTimeout(t); controller.abort(); };
  }, [query]);

  // Add food from a search item
  const addFoodFromSearch = async (item, qtyLocal=1, unitLocal='serving') => {
    try {
      const r = await fetch(`/api/food/${item.id}`);
      const details = await r.json();
      // Extract base per-serving or per-100g nutrients
      const base = extractNutrients(details);

      // Determine default base size
      const size = Number(details.servingSize) || 100;
      const sizeUnit = (details.servingSizeUnit || 'g').toString().toLowerCase();
      // Compute multiplier
      let multiplier = 1;
      if (unitLocal === 'g') {
        const gramsBase = sizeUnit.includes('g') ? size : 100;
        multiplier = gramsBase > 0 ? (qtyLocal / gramsBase) : (qtyLocal / 100);
      } else {
        multiplier = qtyLocal; // per serving
      }

      const scaled = Object.fromEntries(Object.entries(base).map(([k,v]) => [k, (Number(v)||0) * multiplier]));
      const entry = {
        id: `${item.id}-${Date.now()}`,
        fdcId: item.id,
        description: item.description,
        brand: item.brand,
        qty: qtyLocal, unit: unitLocal,
        nutrients: scaled
      };
      const dayLog = log[todayKey] ? [...log[todayKey]] : [];
      dayLog.push(entry);
      setLog({ ...log, [todayKey]: dayLog });
    } catch (e) {
      console.error('addFoodFromSearch error', e);
      alert('Failed adding food item. Check console for details.');
    }
  };

  // Remove item
  const removeItem = (id) => {
    const dayLog = (log[todayKey] || []).filter(it => it.id !== id);
    setLog({ ...log, [todayKey]: dayLog });
  };

  // Totals
  const totals = (log[todayKey] || []).reduce((acc,it)=>{
    const n = it.nutrients || {};
    return {
      calories: acc.calories + (Number(n.calories)||0),
      protein:  acc.protein  + (Number(n.protein)||0),
      carbs:    acc.carbs    + (Number(n.carbs)||0),
      fat:      acc.fat      + (Number(n.fat)||0),
      fiber:    acc.fiber    + (Number(n.fiber)||0),
      sodium:   acc.sodium   + (Number(n.sodium)||0),
    };
  }, {calories:0,protein:0,carbs:0,fat:0,fiber:0,sodium:0});

  // History 14 days
  const history = [];
  for (let i=13;i>=0;i--) {
    const d = new Date(); d.setDate(d.getDate()-i);
    const k = formatDateKey(d);
    const dayTotal = (log[k]||[]).reduce((s,it)=> s + (Number(it.nutrients?.calories)||0), 0);
    history.push({ date: k.slice(5), calories: Math.round(dayTotal) });
  }

  // Speech
  const startListening = () => {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) { alert('Speech recognition not supported in this browser.'); return; }
    const recognition = new SpeechRecognition();
    recognition.lang = 'en-US'; recognition.interimResults = false; recognition.maxAlternatives = 1;
    recognition.onresult = async (e) => {
      const transcript = e.results[0][0].transcript;
      const parsed = parseVoiceItems(transcript);
      for (const p of parsed) {
        setQuery(p.name);
        await new Promise(r => setTimeout(r, 350));
        const res = await fetch(`/api/search?q=${encodeURIComponent(p.name)}`);
        const data = await res.json();
        const top = (data.items || [])[0];
        if (top) await addFoodFromSearch(top, p.qty, p.unit || 'serving');
      }
    };
    recognition.onerror = (err) => { console.error('speech error', err); };
    recognition.onend = ()=> setListening(false);
    recognition.start(); recognitionRef.current = recognition; setListening(true);
  };
  const stopListening = () => { if (recognitionRef.current) recognitionRef.current.stop(); setListening(false); };

  // Editable goals
  const [editingGoals, setEditingGoals] = useState(false);
  const [editGoalsState, setEditGoalsState] = useState(goals || {});
  useEffect(()=>{ if (goals) setEditGoalsState(goals); }, [goals]);
  const onChangeGoal = (k, val)=> setEditGoalsState(prev=>({ ...prev, [k]: Math.max(0, Number(val)||0) }));
  const saveGoals = ()=> { setGoals(editGoalsState); setEditingGoals(false); };
  const cancelGoals = ()=> { setEditGoalsState(goals); setEditingGoals(false); };

  return (
    <main className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-amber-50 p-4">
      <div className="max-w-xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold">QuickFood 🍌</h1>
          <p className="text-sm text-gray-600">Speak or type — log quickly.</p>
        </header>

        {!goals && <GoalWizard onSave={(g)=>setGoals(g)} />}

        {goals && (
          <section className="bg-white p-4 rounded-xl shadow mb-4">
            <div className="flex justify-between items-start">
              <div>
                <div className="text-sm text-gray-500 flex items-center gap-2">
                  Calories {!editingGoals && (<button onClick={()=>setEditingGoals(true)} className="text-xs text-blue-600 underline">Edit goals</button>)}
                </div>
                {!editingGoals ? (
                  <div className="text-2xl font-bold">{Math.round(totals.calories)} / {goals.calories} kcal</div>
                ) : (
                  <input type="number" className="w-24 border p-1 rounded" value={editGoalsState.calories||0} onChange={(e)=>onChangeGoal('calories', e.target.value)} />
                )}
              </div>
              <div className="text-right text-sm text-gray-600 space-y-1">
                {['protein','carbs','fat','fiber','sodium'].map(key => (
                  <div key={key} className="flex items-center gap-2 justify-end">
                    <span className="capitalize" style={{width:'70px'}}>{key}</span>
                    {!editingGoals ? (
                      <span>{Math.round(totals[key])} / {goals[key]}{key==='sodium'?' mg':' g'}</span>
                    ) : (
                      <input type="number" className="w-20 border p-1 rounded" value={editGoalsState[key]||0} onChange={(e)=>onChangeGoal(key, e.target.value)} />
                    )}
                  </div>
                ))}
              </div>
            </div>

            <div className="mt-3 space-y-2">
              <ProgressBar label="Calories" value={totals.calories} target={editingGoals ? editGoalsState.calories : goals.calories} />
              <ProgressBar label="Protein (g)" value={totals.protein} target={editingGoals ? editGoalsState.protein : goals.protein} />
              <ProgressBar label="Carbs (g)" value={totals.carbs} target={editingGoals ? editGoalsState.carbs : goals.carbs} />
              <ProgressBar label="Fat (g)" value={totals.fat} target={editingGoals ? editGoalsState.fat : goals.fat} />
              <ProgressBar label="Fiber (g)" value={totals.fiber} target={editingGoals ? editGoalsState.fiber : goals.fiber} />
              <ProgressBar label="Sodium (mg)" value={totals.sodium} target={editingGoals ? editGoalsState.sodium : goals.sodium} />
            </div>

            {editingGoals && (
              <div className="flex gap-2 mt-4 justify-end">
                <button onClick={cancelGoals} className="px-3 py-1 border rounded">Cancel</button>
                <button onClick={saveGoals} className="px-3 py-1 bg-emerald-500 text-white rounded">Save Goals</button>
              </div>
            )}
          </section>
        )}

        <section className="bg-white rounded-xl p-3 shadow mb-4">
          <div className="flex gap-2">
            <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search foods or tap mic" className="flex-1 p-3 rounded-xl border" />
            <button onClick={()=>{ listening ? stopListening() : startListening(); }} className="px-4 rounded-xl bg-rose-500 text-white" aria-label="Start voice input">
              {listening ? 'Listening…' : '🎤'}
            </button>
          </div>

          {searchResults.length>0 && (
            <ul className="mt-2 max-h-48 overflow-auto">
              {searchResults.map(it => (
                <li key={it.id} className="p-2 hover:bg-gray-50 flex justify-between items-center">
                  <div>
                    <div className="font-medium">{it.description}</div>
                    <div className="text-xs text-gray-500">{it.brand || it.dataType}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <input type="number" value={qty} onChange={(e)=>setQty(Number(e.target.value))} className="w-20 p-1 border rounded" />
                    <select value={unit} onChange={(e)=>setUnit(e.target.value)} className="p-1 border rounded">
                      <option value="serving">serving</option>
                      <option value="g">g</option>
                    </select>
                    <button onClick={()=>addFoodFromSearch(it, qty, unit)} className="px-3 py-1 bg-emerald-500 text-white rounded">Add</button>
                  </div>
                </li>
              ))}
            </ul>
          )}
        </section>

        <section className="bg-white rounded-xl p-3 shadow mb-8">
          <div className="flex justify-between items-center mb-2">
            <h2 className="font-semibold">Today — {new Date().toLocaleDateString()}</h2>
            <div className="text-sm text-gray-500">Calories: <strong>{Math.round(totals.calories)}</strong></div>
          </div>
          <div className="space-y-2">
            {(log[todayKey]||[]).map(it => (
              <div key={it.id} className="flex justify-between items-center p-2 border rounded">
                <div>
                  <div className="font-medium">{it.description}</div>
                  <div className="text-xs text-gray-500">{it.qty} {it.unit}</div>
                </div>
                <div className="text-right">
                  <div className="text-sm">{Number.isFinite(it.nutrients?.calories) ? Math.round(it.nutrients.calories) : 0} kcal</div>
                  <button onClick={()=>removeItem(it.id)} className="text-xs text-red-500 mt-1">Delete</button>
                </div>
              </div>
            ))}
            {(log[todayKey]||[]).length===0 && <div className="text-sm text-gray-500">No items yet — add something!</div>}
          </div>
        </section>

        <section className="bg-white rounded-xl p-3 shadow mb-8">
          <h3 className="font-semibold mb-2">History (last 14 days)</h3>
          <HistoryChart data={history} />
        </section>

        <footer className="text-center text-xs text-gray-600">
          Data: USDA FoodData Central. Speech uses browser SpeechRecognition API.
        </footer>
      </div>
    </main>
  );
}
